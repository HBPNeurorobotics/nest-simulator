...
music_cont_out_proxy::calibrate()
{
  ...
  if ( S_.published_ == false )
  {
    ...
    {
      if ( NestModule::get_network().is_local_gid( P_.target_gids_[ i ] ) )
      {
        for ( long_t j = 0; j < P_.record_from_.size(); ++j )
        {
    ...
    MUSIC::ArrayData* dmap = new MUSIC::ArrayData(
...
music_cont_out_proxy::set_status( const DictionaryDatum& d )
{
  ...
  // State_ stmp = S_;
  S_.set( d ); // throws if BadProperty
}
...
music_cont_out_proxy::handle( DataLoggingReply& reply )
{
  ...
  // const index gid_offset = target_gid_offset_.get( sender_gid );
